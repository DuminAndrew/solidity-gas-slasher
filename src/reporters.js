...
function stylish(reports) {
  ...
  for (const r of reports) {
    ...
    for (const f of r.findings) {
      total++;
      lines.push(
        `  ${f.line}:${f.column}  ${SEV_ICON[f.severity] || '·'} ${f.severity.padEnd(6)} ${f.ruleId}  — ${f.title}`,
      );
...
function markdown(reports) {
  ...
  if (!f.length) {
    L.push('✅ Газ-неэффективностей не найдено.');
    return L.join('\n');
  }
...
function sarif(reports) {
  ...
  for (const r of reports) {
    for (const x of r.findings) {
      if (!rules.has(x.ruleId))
        rules.set(x.ruleId, { id: x.ruleId, name: x.title, shortDescription: { text: x.title } });
      results.push({
        ruleId: x.ruleId,
        level: level[x.severity] || 'warning',
        message: { text: `${x.title}: ${x.message} [${x.gasHint}]` },
        locations: [
          {
            physicalLocation: {
              artifactLocation: { uri: x.file },
              region: { startLine: Math.max(1, x.line), startColumn: Math.max(1, x.column + 1) },
            },
          },
        ],
      });
  ...
  return JSON.stringify(
    {
      ...
      runs: [
        { tool: { driver: { name: 'solidity-gas-slasher', version: '0.1.0', rules: [...rules.values()] } }, results },
      ],
    },
    null,
    2,
