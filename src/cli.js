...
function collectSol(target) {
  ...
  if (st.isFile()) { if (target.endsWith('.sol')) out.push(target); return out; }
  (function walk(d) {
...
function main(argv) {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    else if (a === '-h' || a === '--help') { console.log('gas-slasher <file.sol|dir> [-f stylish|json|sarif|md] [--fail-on low|medium|high] [-o file]'); return 0; }
    else o.target = a;
  }
  if (!o.target) { console.error('Укажите .sol файл или папку. --help — справка.'); return 1; }
  let files;
  try { files = collectSol(o.target); } catch (e) { console.error('Путь не найден: ' + o.target); return 1; }
  if (!files.length) { console.error('.sol файлы не найдены: ' + o.target); return 1; }

  const reports = files.map((f) => analyzeSource(fs.readFileSync(f, 'utf8'), path.relative(process.cwd(), f).split(path.sep).join('/')));
  const reporter = reporters[o.format === 'md' ? 'markdown' : o.format] || reporters.stylish;
  const out = reporter(reports);
  if (o.output) { fs.writeFileSync(o.output, out, 'utf8'); console.log('Отчёт сохранён: ' + o.output); }
  else { try { process.stdout.write(out + '\n'); } catch (e) { console.log(out); } }
